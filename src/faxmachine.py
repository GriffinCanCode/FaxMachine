...
def smart_preview_file(file_path):
    ...
    if not nltk_available and not spacy_available and not yake_available:
        try:
            print_colored(
                "Installing lightweight NLP library for text analysis...", Colors.YELLOW
            )
            ...
            error_message = f"NLP library installation failed: {e!s}"
    ...
    if file_ext == ".pdf" and not pdf_available:
        try:
            ...
            error_message = f"PDF library installation failed: {e!s}"
    ...
    try:
        ...
        if file_ext == ".json":
                    ...
                    if len(all_keys) > 0:
                        ...
                        for key in sorted(set(all_keys))[:5]:
                            if (
                                len(key) > 3
                                and key.isalpha()
                                and key.lower() not in suggested_tags
                            ):
            ...
            if any(
                date_term in header_str
                for date_term in ["date", "time", "year", "month"]
            ):
                suggested_tags.append("temporal")
            if any(
                geo_term in header_str
                for geo_term in ["country", "city", "location", "address", "state"]
            ):
                suggested_tags.append("geographic")
            if any(
                fin_term in header_str
                for fin_term in ["price", "cost", "revenue", "sales", "profit"]
            ):
            ...
            if pdf_available:
                try:
                    ...
                    with redirect_stderr(io.StringIO()):
                        with pdfplumber.open(file_path) as pdf:
                            ...
                            for line in lines[:5]:
                                ...
                                if (
                                    len(clean_line) < 5
                                    or len(clean_line) > 100
                                    or re.search(
                                        r"\d{1,2}[/\.-]\d{1,2}[/\.-]\d{2,4}", clean_line
                                    )
                                    or re.search(
                                        r"http|www|\.com|github|colab",
                                        clean_line.lower(),
                                    )
                                ):
                                    continue
                            ...
                            # If no good candidates, try using the filename as title
                                ...
                                title_from_filename = filename_without_ext.replace(
                                    "_", " "
                                ).replace("-", " ")
                                title_from_filename = re.sub(
                                    r"\s+", " ", title_from_filename
                                ).strip()
                            ...
                            content_for_summary = "\n".join(
                                page_texts[:3] if page_texts else []
                            )

                            # Clean up text for better processing
                            clean_content = re.sub(
                                r"\s+", " ", content_for_summary
                            ).strip()
                            ...
                            word_extraction_text = re.sub(
                                r"[^\w\s]", " ", clean_content.lower()
                            )
                            ...
                            for section in section_candidates:
                                ...
                                if (
                                    len(clean_section) >= 4
                                    and len(clean_section) <= 50
                                    and not re.search(
                                        r"http|www|\.com|github|colab",
                                        clean_section.lower(),
                                    )
                                    and not clean_section.isdigit()
                                    and
                                    # Avoid duplicates
                                    clean_section not in clean_sections
                                    and
                                    # Avoid common headers/footers
                                    not re.match(
                                        r"page \d+|\d+ of \d+|copyright|all rights reserved",
                                        clean_section.lower(),
                                    )
                                ):
                            ...
                            if key_terms:
                                topics_text = "Key topics: " + ", ".join(
                                    [term.capitalize() for term in key_terms[:5]]
                                )
                            ...
                            if clean_sections:
                                ...
                                section_text = "Sections include: " + ", ".join(
                                    clean_sections[:5]
                                )
                                if len(clean_sections) > 5:
                                    section_text += (
                                        f" and {len(clean_sections) - 5} more"
                                    )
                            ...
                            domain_terms = re.findall(
                                r"\b(?:API|SDK|AI|ML|NLP|GPU|CPU|HTTP|REST|Cloud|React|Angular|Vue|Node|Python|Java|SQL|NoSQL|MongoDB|Docker|Kubernetes|AWS|Azure|GCP|IoT|blockchain|data science|machine learning|deep learning|neural network|analytics|visualization)\b",
                                text,
                                re.IGNORECASE,
                            )
                    ...
                    error_message = f"PDF parsing error: {e!s}"
        ...
        error_message = f"Error analyzing file: {e!s}"
...
def _extract_keywords(text, max_keywords=5):
    ...
    if importlib.util.find_spec("nltk") is not None:
        try:
            ...
            filtered_tokens = [
                w for w in tokens if w.isalpha() and len(w) > 3 and w not in stop_words
            ]
...
def preview_diff(source_path, dest_path):
    ...
    if os.path.exists(dest_path):
        ...
        with open(dest_path, errors="ignore") as f:
...
def inject_file(file_path, dest_name=None, preview=True):
    ...
    try:
        ...
        print_colored(
            f"Injected '{os.path.basename(source_path)}' to current directory",
            Colors.GREEN,
        )
        ...
        config["last_accessed"] = config["last_accessed"][
            :10
...
def interactive_file_browser():
    ...
    while True:
        ...
        if select_multiple_mode:
            multiselect_header = (
                f"MULTI-SELECT MODE: {len(selected_files)} files selected"
            )
            print_colored("=" * len(multiselect_header), Colors.BG_BLUE + Colors.BOLD)
            print_colored(
                multiselect_header, Colors.BG_BLUE + Colors.BOLD + Colors.BLACK
            )
            print_colored("=" * len(multiselect_header), Colors.BG_BLUE + Colors.BOLD)
            print_colored(
                "Select files using their numbers. Press 'a' to process selected files.",
                Colors.BLUE,
            )
        ...
        try:
            ...
            files = sorted(
                [f for f in items if os.path.isfile(os.path.join(current_path, f))]
            )
            ...
            print_colored(f"Error: {e!s}", Colors.RED)
            current_path = os.path.dirname(current_path) or os.getcwd()
            continue
        ...
        for i, f in enumerate(files):
            ...
            if select_multiple_mode:
                ...
                if file_path in selected_files:
                    selected_marker = (
                        Colors.BG_GREEN + Colors.BLACK + " [✓] " + Colors.ENDC
                    )
            ...
            if in_database:
                ...
                print(
                    f"  {file_idx}. {selected_marker}{f} {ext_str} ({size_str}){desc}"
                )
            else:
            ...
            if expanded_file_idx is not None and file_idx == expanded_file_idx:
                ...
                try:
                    ...
                    print(
                        ...
                        + " " * (46 - len(f"Error generating preview: {e!s}"))
                        + Colors.BLUE
                        + " │"
                        + Colors.ENDC
                    )
        ...
        if choice == "q":
            ...
            if select_multiple_mode:
                ...
                print_colored(
                    ...
                    Colors.BG_BLUE + Colors.BOLD + Colors.BLACK,
                )
                print_colored("=" * 40, Colors.BG_BLUE + Colors.BOLD)
                print_colored(
                    "• Use file numbers to select/deselect files",
                    Colors.BLUE + Colors.BOLD,
                )
                print_colored(
                    "• Press 'a' to perform actions on selected files",
                    Colors.BLUE + Colors.BOLD,
                )
                print_colored(
                    "• Press 'c' to clear all selections", Colors.BLUE + Colors.BOLD
                )
                print_colored(
                    "• Press '*' or 'm' again to exit multi-select",
                    Colors.BLUE + Colors.BOLD,
                )
                input("\nPress Enter to continue...")
            else:
                ...
                print_colored(
                    ...
                    Colors.BG_YELLOW + Colors.BOLD + Colors.BLACK,
                )
            ...
            try:
                ...
                if file_num_str:
                    ...
                    print_colored(
                        "Please provide a file number with 'd', like 'd5'",
                        Colors.YELLOW,
                    )
                    input("\nPress Enter to continue...")
            except ValueError:
            ...
            if len(selected_files) > 0:
                ...
                if in_database:
                    ...
                    if action == "1":
                        ...
                        if not category:
                            print_colored(
                                "Category required for adding files", Colors.RED
                            )
                        else:
                            for file_path in selected_files:
            ...
            if in_database:
                print_colored(
                    "Cannot mass add from database mode. Toggle to system mode first.",
                    Colors.YELLOW,
                )
                input("\nPress Enter to continue...")
                continue
            ...
            if search_term:
                ...
                if found_items:
                    ...
                    if search_choice.isdigit() and 0 < int(search_choice) <= len(
                        found_items[:20]
                    ):
            ...
            while True:
                ...
                print_colored(
                    "\nNavigate: j/k (down/up), h (parent), l (enter), q (quit vim mode)",
                    Colors.BOLD,
                )
                ...
                if key == "q":
                    ...
                    if os.path.isdir(full_path):
                        try:
                            ...
                            print_colored(
                                "Permission denied to access this directory", Colors.RED
                            )
                            input("\nPress Enter to continue...")
                        except Exception as e:
                            print_colored(f"Error: {e!s}", Colors.RED)
...
def _curses_file_browser(in_db=False, shortcuts=None):
    ...
    try:
        ...
        def draw_menu(
            ...
            stdscr.addstr(
                1, 0, f"Mode: {mode_text} | Path: {current_path}", curses.A_NORMAL
            )
            ...
            if select_multiple_mode:
                ...
                try:
                    ...
                    stdscr.addstr(
                        3,
                        0,
                        "Press SPACE to select/deselect | 'a' to process | 'm' to exit",
                        curses.A_NORMAL,
                    )
                except:
            ...
            if (
                expanded_info
                and expanded_info[0] >= start_idx
                and expanded_info[0] < start_idx + visible_items
            ):
            ...
            for i in range(start_idx, end_idx):
                ...
                if i == cursor_pos:
                    ...
                    if is_selected:
                        try:
                            ...
                            stdscr.addstr(
                                current_row, 0, f"> {display_str}", curses.A_REVERSE
                            )
                    else:
                        stdscr.addstr(
                            current_row, 0, f"> {display_str}", curses.A_REVERSE
                    ...
                    if expanded_info and expanded_info[0] == i and not is_dir:
                        ...
                        if available_rows > 0:
                            ...
                            if (
                                summary
                                and current_row < height
                                and rows_used < available_rows
                            ):
                                # Wrap summary text to fit box
                                wrapped_summary = textwrap.wrap(
                                    summary, summary_width - 4
                                )
                                if wrapped_summary:
                                    try:
                                        summary_text = wrapped_summary[0][
                                            : summary_width - 6
                                        ]
                                        padding = " " * (
                                            summary_width - len(summary_text) - 6
                                        )
                                        stdscr.addstr(
                                            current_row,
                                            2,
                                            "│ " + summary_text + padding + " │",
                                            curses.A_NORMAL,
                                        )
                                    except:
                                        pass
                            ...
                            if (
                                ...
                                try:
                                    padding = " " * (
                                        summary_width - len(preview_line) - 6
                                    )
                                    stdscr.addstr(
                                        current_row,
                                        2,
                                        "│ " + preview_line + padding + " │",
                                        curses.A_NORMAL,
                                    )
                                except:
                                    pass
                ...
                # Normal items
                    ...
                    try:
                        ...
                        stdscr.addstr(
                            current_row, 0, f"  {display_str}", curses.A_NORMAL
                        )
                else:
        ...
        def main(stdscr):
            ...
            while True:
                ...
                draw_menu(
                    ...
                    select_multiple_mode=select_multiple_mode,
                ...
                if key == ord("q"):
                    ...
                    key in [ord(str(i)) for i in range(1, 10)]
                    and shortcuts
                    and not in_db
                ):
                    ...
                    if in_db and (
                        ...
                        or parent == os.path.dirname(os.path.dirname(DB_DIR))
                    ):
                        current_path = DB_DIR
        ...
        print_colored(f"Error in curses interface: {e!s}", Colors.RED)
    ...
    if args.list_only:
        ...
        if results:
            ...
            if choice.startswith("v") and choice[1:].isdigit():
                ...
                if 0 <= idx < len(results):
                    dest_name = input(
                        "Enter destination filename (leave blank for original): "
                    )
                    if not dest_name:
                        dest_name = None
...
def mass_add_files():
    ...
    try:
        ...
        file_paths = filedialog.askopenfilenames(
            title="Select files to add to Faxmachine", multiple=True
        )
        ...
        print_colored(
            "File selection dialog not available, using manual input", Colors.YELLOW
        )
        files_to_add = []
    ...
    use_smart_preview = input(
        "Use smart content analysis to generate summaries and tag suggestions? [Y/n] "
    )
    use_preview = use_smart_preview.lower() != "n"

    common_desc = input(
        "Enter common description for all files (optional, press Enter to set individually): "
    )
    common_tags = input(
        "Enter common tags for all files (comma-separated, optional, press Enter to set individually): "
    )
    common_tags_list = (
        [t.strip() for t in common_tags.split(",")] if common_tags else None
    )
    ...
    for i, file_path in enumerate(files_to_add):
        print_colored(
            f"\nProcessing file {i+1}/{len(files_to_add)}: {os.path.basename(file_path)}",
            Colors.BOLD,
        )
        ...
        if desc is None or desc == "":
            if auto_summary:
                ...
                desc = input(
                    f"Enter description for '{os.path.basename(file_path)}' (optional): "
                )
        ...
        if tags_list is None:
            if suggested_tags:
                ...
                tags = input(
                    f"Enter tags for '{os.path.basename(file_path)}' (comma-separated, optional): "
                )
                tags_list = [t.strip() for t in tags.split(",")] if tags else []

        name = input(
            f"Enter name for '{os.path.basename(file_path)}' (leave blank for original): "
        )
        if not name:
    ...
    print_colored(
        f"\nAdded {success_count} out of {len(files_to_add)} files successfully",
        Colors.GREEN,
    )
    input("Press Enter to continue...")
...
def main():
    ...
    add_parser.add_argument(
        "-s", "--subcategory", help="Subcategory to add the file to"
    )
    ...
    inject_parser = subparsers.add_parser(
        "inject", aliases=["print"], help="Inject a file from the database"
    )
    ...
    inject_parser.add_argument(
        "--no-preview", action="store_true", help="Skip preview when injecting"
    )
    ...
    search_parser.add_argument(
        "-c", "--content-search", action="store_true", help="Search in file contents"
    )
    search_parser.add_argument("-t", "--tags", help="Filter by comma-separated tags")
    search_parser.add_argument(
        "--list-only", action="store_true", help="Only list results, no prompts"
    )
    ...
    search_parser.add_argument(
        "-n", "--name", help="Name for injected file (with --inject-index)"
    )
    ...
    delete_parser = subparsers.add_parser(
        "delete", help="Delete a file from the database"
    )
    ...
    # Mass add command
    mass_add_parser = subparsers.add_parser(
        "mass-add", help="Add multiple files at once"
    )

    # Document viewer command
    doc_viewer_parser = subparsers.add_parser(
        "browser", aliases=["view"], help="Browse files with smart preview"
    )
    doc_viewer_parser.add_argument(
        "file", nargs="?", help="Path to the document to view"
    )
